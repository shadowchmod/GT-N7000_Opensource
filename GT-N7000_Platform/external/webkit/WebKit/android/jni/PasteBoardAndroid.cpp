...
WebCore::String getText(jweak m_obj){
...
WebCore::String getHTML(jweak m_obj){
    JNIEnv* env = JSC::Bindings::getJNIEnv();
    jstring returnVal = (jstring) env->CallObjectMethod(m_javaGlueForPasteBoard.object(env, m_obj).get(),
...
void setDataToClipBoard(WebCore::String& format , WebCore::String& data, jweak m_obj){
    JNIEnv* env = JSC::Bindings::getJNIEnv();
...
PassRefPtr<DocumentFragment> Pasteboard::documentFragment(Frame* frame, PassRefPtr<Range> context, bool allowPlainText, bool& chosePlainText)
{
	...
	if(!markup.isEmpty()) {	
		...
		String markup = android::getText(clipObj);//ChromiumBridge::clipboardReadPlainText(buffer);
		if (!markup.isEmpty()) {
			chosePlainText = true;
