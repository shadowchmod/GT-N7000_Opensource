...
static inline float textMultiplier(float specifiedSize, float trr)
{
    return max((1.0f / log10f(specifiedSize) * trr), 1.0f);
}    
        ...
        if (isVisibleRenderText(descendent) && resizeTextPermitted(descendent)) {
            ...
            if (scaledSize > 0 && scaledSize < minFontSize) {
                ...
                if(document()->settings()){	
                    trr = document()->settings()->TextReadabilityRatio();
                    LOGE("TR ratio tr=%f",trr);
                }
                candidateNewSize = roundf(min(minFontSize, specifiedSize * textMultiplier(specifiedSize, trr)));
