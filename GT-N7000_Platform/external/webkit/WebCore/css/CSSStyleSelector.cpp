...
//SAMSUNG CHANGE : adding browser webview color changing mode for power saving >>
	Settings* settings = e->document()->settings();
	if( settings->backgroundColor() != 0 ) // if power save mode is enabled
	{
	    	if (alphaChannel(m_style->backgroundColor().rgb()) == 0xFF) {
			int redcolor = redChannel(m_style->backgroundColor().rgb());
			int greencolor = greenChannel(m_style->backgroundColor().rgb());
			int bluecolor = blueChannel(m_style->backgroundColor().rgb());

			if (((245 < redcolor) || (redcolor <= 255)) 
				&& ((245 < greencolor) || (greencolor<= 255)) 
				&& ((245 < bluecolor) || (bluecolor<= 255))){
   				m_style->setBackgroundColor(settings->backgroundColor());
			}
	    	}
	}
//SAMSUNG CHANGE : adding browser webview color changing mode for power saving >>
