...
#if _SAMSUNG_FLASH_OPT_
void FrameView::showPlugin()
{
...
#if _SAMSUNG_FLASH_OPT_ 
//Page Load performance improvement - We do not download any plug-in till 
//page load is completed - begin
    Frame* mainFrame = frame()->page() ? frame()->page()->mainFrame() : 0 ;
    if ( (mainFrame && mainFrame->document() && mainFrame->document()->isPluginDocument()) 
        || !(mainFrame && mainFrame->loader()->isLoading()) ) {
#endif
        for (unsigned i = 0; i < maxUpdateWidgetsIterations; i++) {
            if (updateWidgets())
                break;
