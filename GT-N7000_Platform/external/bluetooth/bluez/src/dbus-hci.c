							...
							uint8_t *auth)
{
	...
	char addr[18];
	int err, dd, dev_id;
//Broadocm Bluetooth feature - CTS VERI START
	struct btd_adapter *adapter;
	struct btd_device *device;
...
#ifdef BT_ALT_STACK
void hcid_dbus_amp_state_changed(bdaddr_t *local, bdaddr_t *peer, uint32_t app_id, uint32_t amp_state)
{
	...
	adapter_emit_amp_state_changed(adapter, app_id, peer, amp_state);
}
...
//Broadocm Bluetooth feature - CTS VERI
	...
	if (new_key_type == 0x03) {
		...
		device_set_debug_key(device, key);
				...
				(key_type == 0x05) || //Broadocm Bluetooth feature - CTS VERI
