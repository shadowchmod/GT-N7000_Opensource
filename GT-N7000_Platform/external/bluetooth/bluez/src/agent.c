...
//				const char *device_path,
				const struct btd_device *device, // Broadocm Bluetooth feature - CTS VERI
				const char *uuid)
{
	struct agent *agent = req->agent;
	const gchar *device_path = device_get_path(device); //Broadocm Bluetooth feature - CTS VERI
	gboolean bTemporary = device_is_temporary(device);   //Broadocm Bluetooth feature - CTS VERI
	...
	dbus_message_append_args(req->msg,
				...
				DBUS_TYPE_BOOLEAN, &bTemporary, //Broadocm Bluetooth feature - CTS VERI
				DBUS_TYPE_INVALID);
